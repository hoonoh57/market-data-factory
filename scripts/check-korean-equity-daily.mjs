...
try {
  const result = await withMySqlConnection(async connection => {
    const [[health]] = await connection.query('SELECT * FROM korean_equity_daily_health');
  ...
  console.log(`[PASS] korean-equity-daily rows=${result.health.row_count} instruments=${result.health.instrument_count} range=${String(result.health.earliest_trading_date).slice(0,10)}..${String(result.health.latest_trading_date).slice(0,10)}`);
} catch (error) {
